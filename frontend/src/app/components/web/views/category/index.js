...
const Category = () => {
    ...
    //         });
    // }, []); // Thêm một mảng rỗng để useEffect chỉ chạy một lần sau khi component được render lần đầu tiên

     useEffect(() => {
        callFetchCategory() // goi lại hàm call api trong file config/api.js
            .then(response => {
                setCategories(response.data);
            })
            .catch(error => {
                ...
                <Slider {...settings}>
                    {categories.map(category => (
                        <div className="item" key={category.id}>
                            <div className="category-item">
                                <Link to={{ pathname: `/shop/${category.slug}` }}>
                                    <img className="img-fluid" src="img/category/household.png" alt={category.name} />
                                    <h6>{category.name}</h6>
                                </Link>
                            </div>
                        </div>
